function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  plainTextPositions.sort((x, y) => y.start - x.start);

  for (const pos of plainTextPositions) {
    const highlightedWord = plainText.substring(pos.start, pos.end);
    const startIndex = htmlContent.indexOf(highlightedWord);
    const endIndex = startIndex + highlightedWord.length;
    const newPos = htmlContent.slice(0, startIndex) +
      `<mark>${highlightedWord}</mark>` +  htmlContent.slice(endIndex);
    htmlContent = newPos;
  }
